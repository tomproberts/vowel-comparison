...
PLOT_TITLE <- "Vowel Space of Clone, Monophthongs, \"Die Sonne und der Wind\""
...
my_vowels$avg_b1 <- eval(v_to_bark(my_vowels$avg_f1), my_vowels)
my_vowels$avg_b2 <- eval(v_to_bark(my_vowels$avg_f2), my_vowels)

# Plot
p <- my_vowels %>%
  ggplot(.)+
  aes(x = avg_b2, y = avg_b1, color = phone, label = phone)+
